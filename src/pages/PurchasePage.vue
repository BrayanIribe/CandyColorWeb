    ...
    <IContainer>
      ...
      <div>
        <b-modal
          ...
          ref="customerModal"
          size="xl"
          :hide-header-close="proveedor.id <= 0"
          :no-close-on-esc="proveedor.id <= 0"
          :no-close-on-backdrop="proveedor.id <= 0"
          hide-footer
        >
          <b-alert variant="info" :show="proveedor.id <= 0"
            >Por favor seleccione un proveedor para hacer una nota de
            venta.</b-alert
          >
          <SuppliersPage @onSelectedRow="onCustomerSelected" :isModal="true" />
        </b-modal>
        ...
        <div class="row">
          <div class="col-10">
            <span v-if="proveedor.id > 0">Proveedor: {{ $customer }}</span>
          </div>
          <div class="col-2">
            <b-btn
              class="i-new-btn"
...
export default {
  components: { IHomeHeader, IContainer, SuppliersPage },
  data() {
    return {
      documento: {
  ...
  computed: {
    ...
    $customer() {
      const { id, nombre } = this.proveedor;
      return `${id} - ${nombre}`;
    },
  },
  methods: {
    ...
    onCustomerSelected(customer) {
      ...
      proveedor.id = customer.id;
      proveedor.nombre = customer.nombre;
    ...
    async submit() {
      ...
      const { proveedor, documento, conceptos, $total, observaciones } = this;
      ...
      if (proveedor.id <= 0) {
        await this.$error("Debe seleccionar un proveedor.");
        this.$refs.customerModal.show();
        return;
      }

      if (!conceptos || conceptos.length <= 0) {
        await this.$error("Debe tener productos la compra.");
        return;
      }

      if ($total <= 0) {
        this.$error("No puede terminar la compra con un total de ceros.");
        return;
      }
      ...
      if (response.status === 201) {
        await this.$ok("Se ha registrado la compra con éxito.");
        this.$router.go();
      } else {
        await this.$error(
