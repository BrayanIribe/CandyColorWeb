  ...
  methods: {
    ...
    async submit() {
      ...
      if (!conceptos || conceptos.length <= 0) {
        await this.$error("Debe tener productos la pedido.");
        return;
      }

      if ($total <= 0) {
        this.$error("No puede terminar la pedido con un total de ceros.");
        return;
      }

      const result = await this.$swal({
        title: "¿Desea terminar la pedido?",
      ...
      if (response.status === 201) {
        await this.$ok("Se ha registrado la pedido con éxito.");
        this.$router.go();
      } else {
        await this.$error(
